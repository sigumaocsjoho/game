...
function resizeCanvas() {
  ...
  if (isMobile) {
    uiScale = Math.min(WIDTH / 800, HEIGHT / 450);

    uiButtonSize = Math.max(70, 90 * uiScale);
    uiFontSize = Math.max(18, 22 * uiScale);
  } else {
    uiScale = 1;
    uiButtonSize = 60;
    uiFontSize = 20;
...
backgroundImg.src = "haikei/haikei2.jpg";
...
const badgeSrcList = [
  "animals/hamusuta--02.png",
  "animals/doubutu_kinouedenerukoara.png",
  "animals/shibainu-tyairo-1024x1024.png",
  "animals/ac02cd7f7b10e4a82de16f77d7ff85e7_t.png",
  "animals/pngtree-outstretched-uncommon-falcon-clip-art-png-image_2946936.png",
  "animals/cat-mike-800x800.png",
  "animals/youkai_jinmenken.png",
  "animals/hamster_sleeping_golden.png",
  "animals/dog_great_dane.png",
  "animals/animal_black_sheep_hitsuji.png",
  "animals/animal_rakuda_kobu_tareru.png",
  "animals/animal_uma_horse_stand.png",
  "animals/animal_wallaby_kangaroo.png",
  "animals/animalhealth_niwatori_byouki.png",
  "animals/animalhealth_niwatori_genki.png",
  "animals/animal_okojo_summer.png",
  "animals/animal_moose.png",
  "animals/animal_hitsuji_shiboubi.png"
];
...
function landOnBadge(falling, base) {
  ...
  // ★ 落下中 = onGround false
  if (falling.onGround || falling.vy < 0) return false;
  ...
  for (let loop = 0; loop < MAX_LOOP; loop++) {
    ...
    for (let i = 0; i < badges.length; i++) {
      for (let j = i + 1; j < badges.length; j++) {
        ...
        const minDist = (a.size + b.size) / 2 * 1.02;
...
function updateBadges() {
  // ★ 落下処理対象（空中で落下中）
  const fallingBadges = badges.filter(b => b !== heldBadge && b.isFalling);

  /* ===============================
     1) 落下（重力）
  =============================== */
  for (const b of fallingBadges) {
    b.vy += 0.8;
    b.y += b.vy;
  }

  /* ===============================
     2) バッジ同士の着地（落下中→地面上のバッジ）
  =============================== */
  for (const falling of fallingBadges) {
    if (falling.onGround) continue;

    for (const base of badges) {
      if (falling === base) continue;
      if (base === heldBadge) continue;
  ...
  for (const b of fallingBadges) {
    ...
    if (landOnGround(b)) {
      b.y = getGroundY(cx) - b.size;
      b.onGround = true;
      b.vy = 0;
      b.isFalling = false;

      // ★ ここで vx を 0 にしてもOK（坂の力で動き出す）
      // b.vx = 0;
    }
  }
  ...
  /* ===============================
     4) 地面上の転がり（★ここが重要）
     → isFalling=false でも onGround=true なら毎フレーム滑る
  =============================== */
  const groundBadges = badges.filter(b => b !== heldBadge && b.onGround);

  for (const b of groundBadges) {
    const cx = b.x + b.size / 2;
    ...
    b.vx += rollForce * 0.6; // 斜面加速
    b.vx *= 0.96;            // 摩擦

    if (Math.abs(b.vx) < 0.01) b.vx = 0;

    const nextX = b.x + b.vx;
    const nextCX = nextX + b.size / 2;
    const nextGround = getGroundY(nextCX);

    // 上り坂へ“めり込んで登る”のを防止（元の安全策）
...
function isOverlapping(a, b, margin = 30) {
  return !(
...
// --- スポーン専用：横方向だけを見る軽量重なり判定 ---
function isOverlappingSpawn(a, b) {
  const dx = Math.abs(
    (a.x + a.size / 2) - (b.x + b.size / 2)
  );
  return dx < (a.size + b.size) * 0.6;
}
...
function generateBadges() {
  ...
  const BADGE_COUNT = 15;

  // ★ 生成時の重なり防止（2D距離）
  function isOverlappingSpawn2D(a, b, margin = 8) {
    const ax = a.x + a.size / 2;
    ...
    const minDist = (a.size + b.size) / 2 + margin;

    return dist < minDist;
  }

  // ★ 上1/3に出さないための下限Y
  const TOP_LIMIT_Y = HEIGHT / 3;

  // ★ 浮かせる最小/最大オフセット（最大は地形とTOP_LIMIT_Yから計算してさらに上限をかける）
  const MIN_OFFSET = 80;
  const MAX_OFFSET_CAP = 420; // これ以上高くしない（安全上限）
  ...
  for (let i = 0; i < BADGE_COUNT; i++) {
    ...
    while (!safe && tries < 160) {
      ...
      do {
        x = rand(50, WIDTH - 200);
        cx = x + size / 2;
      } while (cx > dropLeft && cx < dropRight);

      const groundY = getGroundY(cx);

      // ★ y = groundY - size - offset が TOP_LIMIT_Y より上にならないように
      //    offset <= groundY - size - TOP_LIMIT_Y
      const maxOffsetByTopLimit = Math.floor(groundY - size - TOP_LIMIT_Y);
      const maxOffset = Math.min(MAX_OFFSET_CAP, maxOffsetByTopLimit);

      // もし条件を満たすオフセットが取れない（地面が低い等）なら、この試行はやり直し
      if (maxOffset < MIN_OFFSET) continue;

      const spawnOffsetY = rand(MIN_OFFSET, maxOffset + 1); // +1 で上限含める

      badge = {
        x,
        y: groundY - size - spawnOffsetY, // ★ 空中だが上1/3より上には行かない
        size,
        img: imgs[rand(0, imgs.length)],
        isRare: Math.random() < 0.2,

        vx: 0,
        vy: 0,
        onGround: false,
        isFalling: false, // ★ 開始時は落下しない
      };

      // ★ 2D判定で重なりを防ぐ
      safe = !badges.some((b) => isOverlappingSpawn2D(b, badge, 10));
    }
...
if (!clawMovingDown && !clawMovingUp && !movingToDropZone && !droppingBadge && !returningToStart) {
  if (!showingBadge && showBadgeTimer <= 0) {
    ...
    // ← PC
    if (keys["ArrowLeft"]) craneX -= craneSpeed;
    if (keys["ArrowRight"]) craneX += craneSpeed;

    // ← ★ スマホ
    if (touchLeft) craneX -= craneSpeed;
    if (touchRight) craneX += craneSpeed;
...
if (clawMovingDown) {
    ...
    if (!heldBadge) {
      for (let badge of badges) {
        if (
          !badge.isFalling &&        // ★ 落下中じゃなければ掴める（空中停止もOK）
          isColliding(
            craneX - 15,
            clawY,
            30,
...
if (Math.random() < slipChance) {
    ...
    heldBadge.sleeping = false;
    heldBadge.stillFrames = 0;
    heldBadge.vy = 0;
    heldBadge.vx = 0;
...
// --- 戻るボタン ---
const backButton = { x: 20, y: 20, width: 160, height: 70, text: "← 戻る" };
